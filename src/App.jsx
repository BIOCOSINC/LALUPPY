...
async function compressImage(file){
  return new Promise(res=>{
    ...
    r.onload=e=>{const img=new Image();img.onload=()=>{const MAX=600,q=0.55;let w=img.width,h=img.height;if(w>MAX){h=Math.round(h*MAX/w);w=MAX;}const c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(img,0,0,w,h);res(c.toDataURL("image/jpeg",q));};img.src=e.target.result;};r.readAsDataURL(file);
  });
}
...
export default function App(){
  ...
  const saveExtraQuota = async (suppId, extraCount) => {
    const key = extraQuotaKey(prodGrade, prodCha, prodYear, prodMonth);
    const data = await db.get(key) || {};
    if(extraCount <= 0) { delete data[suppId]; }
    else { data[suppId] = extraCount; }
  ...
  const loadMyQuota = async () => {
    ...
    const extra = data[me.id] || 0;
    setMyQuota(base + extra);
  };
  ...
  const loadCanSelect=async()=>{
    ...
    if(me.grade===G.L){
      ...
      const prevAct=await db.get(prevActKey);
      if(!prevAct){setCanSelectProduct(true);setCanSelectReason("");return;}
      if(prevAct.submitted){setCanSelectProduct(true);setCanSelectReason("");}
      else{setCanSelectProduct(false);setCanSelectReason(prevCha+"차 활동을 먼저 제출해 주세요.");}
    } else {
      ...
      const last=sels[0];
      if(!last){setCanSelectProduct(true);setCanSelectReason("");return;}
  ...
  const confirmProductSelection=async()=>{
    ...
    if(me.grade===G.L){
      ...
      sel={productId:prod.id,productName:prod.name,productCode:prod.code,cha:selectedCha,
           selYear:now.getFullYear(),selMonth:now.getMonth()+1,selDate:dateStr,slot};
    } else {
      key="selection:"+me.id+":"+selYr+":"+selMo+":slot:"+slot;
      sel={productId:prod.id,productName:prod.name,productCode:prod.code,
           year:selYr,month:selMo,selDate:dateStr,slot};
    }
    const ok=await db.set(key,sel);
    setSelConfirmProd(null);
    if(ok){
      await loadMySelections();
      setSelMsg("제품이 신청되었습니다! ✓ ("+slot+"/"+myQuota+"건)");
  ...
  const tag=g=>({display:"inline-block",padding:"2px 10px",borderRadius:20,fontSize:11,fontWeight:700,background:GB[g]||"#EEE",color:GC[g]||"#333"});
  const lbl={fontSize:12,fontWeight:700,color:MUTED,marginBottom:4,display:"block"};
  ...
  const Logo=({dark})=>BRAND.logoUrl
    ...
    :<span style={{fontWeight:900,fontSize:20,color:dark?"#fff":PC}}>{BRAND.logoText}</span>;
  ...
  const ConfirmModal=()=>{
    ...
    return(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
        <div style={{background:CARD,borderRadius:18,padding:28,maxWidth:340,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.18)"}}>
          <div style={{textAlign:"center",marginBottom:20}}>
            <div style={{fontSize:36,marginBottom:8}}>🛍️</div>
            <div style={{fontWeight:800,fontSize:17,marginBottom:6}}>제품 신청 확인</div>
            <div style={{fontSize:13,color:MUTED,lineHeight:1.6}}>신청하시겠습니까?<br/>신청 후 변경은 불가능합니다.</div>
          </div>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={confirmProductSelection} style={{...btn(PC),flex:2,boxShadow:"0 4px 16px rgba(0,70,56,0.25)"}}>✅ 최종 신청</button>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Noto Sans KR',sans-serif",padding:16}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          {BRAND.logoUrl?<img src={BRAND.logoUrl} alt="LALUPPY" style={{height:56,objectFit:"contain",marginBottom:8}}/>:<div style={{fontWeight:900,fontSize:32,color:PC,marginBottom:8}}>{BRAND.logoText}</div>}
        ...
        <div style={{...ctr,paddingTop:20}}>
          ...
          {sp==="activity"&&act&&(<>
            ...
            {isL&&(<div style={card}>
              ...
              {act.blogs.map((b,i)=>(<div key={i} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><label style={lbl}>블로그 {i+1} {i<2&&<span style={{color:"#C0392B"}}>*</span>}</label>{i>=2&&<button onClick={()=>delBlog(i)} style={btn("#FDECEA","#C0392B",true)}>삭제</button>}</div><input style={inp} placeholder="블로그 링크 입력" value={b.link} onChange={e=>updBlog(i,e.target.value)}/></div>))}
            ...
            <div style={{display:"flex",gap:10,position:"sticky",bottom:16}}>
              ...
              <button onClick={()=>doSave(true)} disabled={saving||act.submitted} style={{...btn(act.submitted?"#CCC":PC),flex:2,opacity:saving?0.7:1,cursor:act.submitted?"not-allowed":"pointer",boxShadow:act.submitted?"none":"0 4px 16px rgba(0,0,0,0.15)"}}>
            ...
            {!canSelectProduct?(
              ...
              mySelections.length >= myQuota ? (
                <div style={{padding:"12px 14px",background:"#E8F5E9",borderRadius:10,marginBottom:14,fontSize:13,color:"#2E7D32",fontWeight:600}}>
                  ✅ {myQuota}건 신청 완료! 이번 {isL?"차수":"기간"} 신청이 모두 완료되었습니다.
                </div>
              ) : (
                <div style={{padding:"12px 14px",background:"#E8F5E9",borderRadius:10,marginBottom:14,fontSize:13,color:"#2E7D32",fontWeight:600}}>
            ...
            <div style={card}>
              ...
              {mySelections.length>0&&(
                <div style={{marginBottom:16}}>
                  ...
                  {mySelections.map((sel,i)=>(
                    <div key={i} style={{padding:"10px 14px",background:BRAND.primaryLight,borderRadius:8,marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                        <div style={{fontSize:11,color:PC,fontWeight:700,marginBottom:1}}>{i+1}번째 신청 · {sel.selDate||""}</div>
                        <div style={{fontWeight:700,fontSize:13}}>{sel.productName}</div>
                        <div style={{fontSize:11,color:MUTED}}>코드: {sel.productCode}</div>
                      </div>
                      <span style={{fontSize:18,color:PC}}>✓</span>
                    </div>
                  ))}
                </div>
              )}
              {canAddMore&&(<>
                ...
                {availableProds.length===0
                  ?<div style={{textAlign:"center",color:MUTED,padding:32,fontSize:13}}>선택 가능한 제품이 없습니다.</div>
                  :<div style={{display:"flex",flexDirection:"column",gap:10}}>
                    {availableProds.map(p=>{
                      const alreadySelected=mySelections.some(s=>s.productId===p.id);
                      return(<div key={p.id} onClick={()=>!alreadySelected&&handleSelectProduct(p)}
                        style={{padding:"14px 16px",borderRadius:10,border:"2px solid "+(alreadySelected?"#CCC":BORDER),background:alreadySelected?"#F5F5F5":CARD,cursor:alreadySelected?"not-allowed":"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",opacity:alreadySelected?0.5:1}}>
                        <div><div style={{fontWeight:700,fontSize:14}}>{p.name}</div><div style={{fontSize:12,color:MUTED,marginTop:2}}>코드: {p.code}</div></div>
                        {alreadySelected
                          ?<span style={{fontSize:11,color:MUTED,padding:"3px 8px",borderRadius:6,background:"#E8E8E8"}}>이미 신청됨</span>
          ...
          {sp==="myinfo"&&(<>
            ...
            {myAddress.address&&!addrEditMode?(
              <div style={card}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontWeight:700,fontSize:14}}>등록된 배송 정보</div><button onClick={()=>setAddrEditMode(true)} style={btn(BRAND.primaryLight,PC,true)}>✏️ 수정하기</button></div>
                {[["수령인",myAddress.name],["연락처",myAddress.phone],["우편번호",myAddress.zonecode],["주소",myAddress.address],["상세주소",myAddress.addressDetail||"-"]].map(([label,value])=>(
                  <div key={label} style={{display:"flex",gap:12,padding:"10px 14px",background:"#F9F6F2",borderRadius:8,marginBottom:8}}><span style={{fontSize:12,color:MUTED,minWidth:60,fontWeight:700}}>{label}</span><span style={{fontSize:13,fontWeight:500}}>{value}</span></div>
                ))}
              </div>
            ):(
              <div style={card}>
                {addrEditMode&&(<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontWeight:700,fontSize:14}}>배송 정보 수정</div><button onClick={()=>setAddrEditMode(false)} style={btn("#EEE8E0",TEXT,true)}>← 취소</button></div>)}
                {!addrEditMode&&<div style={{fontSize:13,color:MUTED,marginBottom:14}}>제품 배송을 위한 수령인 정보와 주소를 등록해 주세요.</div>}
                <label style={lbl}>수령인 이름 *</label><input style={inp} placeholder="실명 입력" value={myAddress.name||""} onChange={e=>setMyAddress(a=>({...a,name:e.target.value}))}/>
                <label style={lbl}>연락처 *</label><input style={inp} placeholder="예: 010-1234-5678" value={myAddress.phone||""} onChange={e=>setMyAddress(a=>({...a,phone:e.target.value}))}/>
                <label style={lbl}>주소 검색</label>
                <div style={{display:"flex",gap:8,marginBottom:10}}><input style={{...inp,marginBottom:0,flex:1}} placeholder="우편번호" value={myAddress.zonecode} readOnly/><button onClick={openPostcode} style={{...btn(PC),whiteSpace:"nowrap",padding:"10px 16px"}}>🔍 검색</button></div>
                <input style={inp} placeholder="기본 주소" value={myAddress.address} readOnly/>
                <label style={lbl}>상세 주소</label><input style={inp} placeholder="동/호수 등 상세 주소 입력" value={myAddress.addressDetail} onChange={e=>setMyAddress(a=>({...a,addressDetail:e.target.value}))}/>
                <button onClick={async()=>{await saveAddress();if(myAddress.address)setAddrEditMode(false);}} disabled={addrSaving} style={{...btn(PC),width:"100%",opacity:addrSaving?0.7:1}}>{addrSaving?"저장 중...":"주소 저장"}</button>
          ...
          {sp==="inquiry"&&(<>
            ...
            {myInquiries.length===0?<div style={{...card,textAlign:"center",color:MUTED,padding:32,fontSize:13}}>아직 문의 내역이 없습니다.</div>
              :myInquiries.map(iq=>(<div key={iq.id} style={{...card,borderLeft:"3px solid "+(iq.reply?PC:BORDER)}}>
                ...
                {iq.reply&&(<div style={{marginTop:12,padding:"10px 14px",background:BRAND.primaryLight,borderRadius:8}}><div style={{fontSize:11,fontWeight:700,color:PC,marginBottom:4}}>💬 관리자 답변 · {iq.replyDate}</div><div style={{fontSize:13,whiteSpace:"pre-wrap",lineHeight:1.6}}>{iq.reply}</div></div>)}
              </div>))}
          </>)}
        </div>
      </div>
      ...
      <div style={{...ctr,paddingTop:20}}>
        ...
        {atab==="notices"&&(<>
          <div style={card}>
            ...
            <div style={{display:"flex",background:"#F0EBE4",borderRadius:10,padding:4,marginBottom:14}}>
              {[[G.L,"라루피"],[G.S,"라루피시크릿"]].map(([g,name])=>(<button key={g} onClick={()=>setNGrade(g)} style={{flex:1,padding:"8px 0",border:"none",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:13,background:nGrade===g?CARD:"transparent",color:nGrade===g?GC[g]:MUTED,boxShadow:nGrade===g?"0 1px 4px rgba(0,0,0,0.1)":"none"}}>{name}</button>))}
            </div>
          ...
          {[[G.L,nlp],[G.S,nsc]].map(([g,list])=>(<div key={g}>
            ...
            {list.map(n=>(<div key={n.id} style={{...card,padding:"12px 16px"}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><div style={{flex:1}}><div style={{fontWeight:700,marginBottom:4}}>{n.title}</div><div style={{fontSize:13,color:MUTED,whiteSpace:"pre-wrap"}}>{n.content}</div><div style={{fontSize:11,color:MUTED,marginTop:6}}>{n.date}</div></div><button onClick={()=>delNotice(g,n.id)} style={btn("#FDECEA","#C0392B",true)}>삭제</button></div></div>))}
          </div>))}
        </>)}
        ...
        {atab==="products"&&(<>
          <div style={card}>
            ...
            {prodGrade===G.L&&(<>
              ...
              <OpenChaList refreshKey={openRefresh}/>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:4}}>
                {LACHAS.map(cha=>(<OpenChaToggle key={cha} cha={cha} refreshKey={openRefresh} onToggle={async()=>{await toggleOpenCha(cha);}}/>))}
              </div>
            ...
            {prodGrade===G.S&&(<>
              ...
              <div style={{fontSize:12,fontWeight:700,color:MUTED,marginBottom:8}}>현재 오픈된 달</div>
          ...
          <div style={card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              ...
              <div style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:BRAND.primaryLight,color:PC,fontWeight:700}}>
            ...
            <div style={{background:"#F9F6F2",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
              ...
              <div style={{position:"relative",marginBottom:8}}>
                ...
                <input style={{width:"100%",padding:"8px 10px 8px 32px",border:"1.5px solid "+(extraQuotaSearch?PC:BORDER),borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box",background:"#fff",fontFamily:"inherit"}}
                  placeholder="기수 또는 닉네임으로 검색" value={extraQuotaSearch} onChange={e=>setExtraQuotaSearch(e.target.value)}/>
                {extraQuotaSearch&&<button onClick={()=>setExtraQuotaSearch("")} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:14,color:MUTED,padding:0}}>×</button>}
              </div>
              {extraQuotaSearch.trim().length>=1&&(()=>{
                ...
                if(matches.length===0)return<div style={{fontSize:12,color:MUTED,textAlign:"center",padding:"8px 0"}}>검색 결과가 없습니다.</div>;
                return matches.map(s=>{
                  ...
                  return(
                    <div key={s.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:currentExtra>0?"#E8F5E9":CARD,borderRadius:8,marginBottom:6,border:"1px solid "+(currentExtra>0?"#A5D6A7":BORDER)}}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,fontSize:13}}>{s.gen} · {s.nick}</div>
                        <div style={{fontSize:11,color:MUTED}}>현재 {total}건 신청 가능 (기본 {BASE_QUOTA[s.grade]}+추가 {currentExtra})</div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <button onClick={()=>saveExtraQuota(s.id,Math.max(0,currentExtra-1))} style={{width:26,height:26,borderRadius:6,border:"1.5px solid "+BORDER,background:CARD,cursor:"pointer",fontWeight:700,fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                        <span style={{fontWeight:800,fontSize:15,color:PC,minWidth:18,textAlign:"center"}}>+{currentExtra}</span>
                        <button onClick={()=>saveExtraQuota(s.id,currentExtra+1)} style={{width:26,height:26,borderRadius:6,border:"1.5px solid "+PC,background:BRAND.primaryLight,cursor:"pointer",fontWeight:700,fontSize:15,color:PC,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                      </div>
                    </div>
                  );
                });
            ...
            {loadingEQ&&<div style={{textAlign:"center",padding:12,color:MUTED,fontSize:12}}>불러오는 중...</div>}
            {!loadingEQ&&extraQuotaList.length===0&&(
              <div style={{textAlign:"center",padding:"10px 0",color:MUTED,fontSize:12}}>추가 허용된 회원이 없습니다.</div>
            )}
              ...
              <div key={eq.suppId} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"#E8F5E9",borderRadius:8,marginBottom:6,border:"1px solid #A5D6A7"}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:MUTED}}>
                    기본 {BASE_QUOTA[eq.grade]||"-"}건 + 추가 {eq.extraCount}건 = 총 {(BASE_QUOTA[eq.grade]||0)+eq.extraCount}건
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <button onClick={()=>saveExtraQuota(eq.suppId,Math.max(0,eq.extraCount-1))} style={{width:26,height:26,borderRadius:6,border:"1.5px solid "+BORDER,background:CARD,cursor:"pointer",fontWeight:700,fontSize:15,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                  <span style={{fontWeight:800,fontSize:15,color:PC,minWidth:18,textAlign:"center"}}>+{eq.extraCount}</span>
                  <button onClick={()=>saveExtraQuota(eq.suppId,eq.extraCount+1)} style={{width:26,height:26,borderRadius:6,border:"1.5px solid "+PC,background:BRAND.primaryLight,cursor:"pointer",fontWeight:700,fontSize:15,color:PC,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                </div>
              </div>
            ))}
            {extraQuotaMsg&&<div style={{fontSize:13,color:PC,marginTop:8,fontWeight:700}}>{extraQuotaMsg}</div>}
          </div>
            ...
            {!loadingSum&&supps.filter(s=>(actGen==="전체"||s.gen===actGen)&&(actGrade==="전체"||s.grade===actGrade)).map(s=>{
              ...
              return(<div key={s.id} style={{...card,padding:"14px 16px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:10,height:10,borderRadius:5,background:has?PC:"#DDD"}}/><button onClick={()=>openSuppActs(s)} style={btn(BRAND.primaryLight,PC,true)}>상세 조회</button></div>
                </div>
              </div>);
            })}
          </>):(<>
        ...
        {atab==="inquiries"&&(<>
          {!selInquiry?(<>
            ...
            <div style={card}><div style={{fontWeight:800,fontSize:15,marginBottom:6}}>{selInquiry.title}</div><div style={{fontSize:12,color:MUTED,marginBottom:12}}>{selInquiry.suppName}&nbsp;<span style={tag(selInquiry.grade)}>{GN[selInquiry.grade]}</span>&nbsp;·&nbsp;{selInquiry.date}</div><div style={{fontSize:14,lineHeight:1.7,whiteSpace:"pre-wrap",padding:12,background:"#F9F6F2",borderRadius:8}}>{selInquiry.content}</div></div>
            <div style={card}><div style={{fontWeight:700,fontSize:14,marginBottom:12}}>💬 관리자 답변</div><textarea style={{...inp,minHeight:100,resize:"vertical"}} value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="답변 내용 입력"/><button onClick={sendReply} style={{...btn(PC),width:"100%"}}>답변 등록</button>{replyMsg&&<div style={{textAlign:"center",marginTop:8,fontSize:13,fontWeight:700,color:PC}}>{replyMsg}</div>}</div>
          </>)}
        </>)}
      </div>
    </div>
